...
class AdminActions {
    ...
    // Obtenemos las rutas relativas si se arrastraron carpetas
    ...
    foreach ($_FILES['archivo_gestor']['name'] as $key => $name) {
        ...
        $extra_path = '';
        // Si el archivo viene dentro de una estructura de carpetas (Drag & Drop)
        if (!empty($paths[$key]) && strpos($paths[$key], '/') !== false) {
            $partes_ruta = explode('/', dirname($paths[$key]));
            // Aseguramos que cada carpeta creada por el gestor lleve el prefijo _gs_
        ...
        // El archivo siempre lleva _gs_ porque lo sube el Admin
